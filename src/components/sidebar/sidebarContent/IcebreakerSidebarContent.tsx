...
const IcebreakersSidebarContent = ({ selectedCategories, onCategoriesChange }: IcebreakersSidebarContentProps) => {
    ...
    return (
        <div className="mt-8">
            <h3 className="px-4 text-base font-semibold text-slate-400 uppercase tracking-wider mb-3">
                Categories
            </h3>
                ...
                {categories.map(({ id, label, enabled }) => (
                    <li key={id}>
                        ...
                        >
                            ...
                            <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${selectedCategories.includes(id)
